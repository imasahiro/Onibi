...
| class utility | Regexp.escape、Regexp.union | ◐ | `Regexp.escape` のSymbol/`to_str`/TypeError coercion、hyphen/control-whitespace escape と、文字列 alternatives / 空集合、および compiled pattern の source を扱う `Regexp.union` を実装。compiled pattern の ignorecase、multiline / extended scope、FIXEDENCODING / NOENCODING も保持するが、その他の全オプション互換は未対応。 |
