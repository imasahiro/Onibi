...
static int onibi_vm_call_subprogram(VALUE states, VALUE outgoing, VALUE subprograms,
  ...
  rb_hash_aset(nested, ID2SYM(rb_intern("outgoing")), outgoing);
  rb_hash_aset(nested, ID2SYM(rb_intern("start_edges")), starts);
  long nested_start = 0;
  VALUE captures = Qnil;
  long nested_end = 0;
...
static int onibi_gir_match_captures_seed(VALUE graph, VALUE str, long start,
  ...
  for (long i = 0; i < RARRAY_LEN(starts); i++) {
    ...
    if (onibi_vm_walk_captures(states, outgoing, onibi_hash_value(graph, "subprograms"), str, NUM2LONG(onibi_hash_value(edge, "to")), start,
...
static int onibi_rseq_simple_match(VALUE rseq, VALUE str, long start, long *matched_end) {
  ...
  for (uint32_t i = 0; i < header.state_count; i++) {
    ...
    if (states[i].op == ONIBI_RS_CHAR) {
      const OnibiLiteralDesc *literals = (const OnibiLiteralDesc *)(RSTRING_PTR(blob) + header.descriptors_offset);
      if (literals[states[i].payload].flags != 0) return -1;
    } else if (states[i].op == ONIBI_RS_CLASS) {
      const OnibiClassDesc *classes = (const OnibiClassDesc *)(RSTRING_PTR(blob) + header.classes_offset);
      if (classes[states[i].payload].flags != 0) return -1;
    }
  }
  ...
  while (stack_size > 0) {
    ...
    if (state->op == ONIBI_RS_CHAR || state->op == ONIBI_RS_CLASS || state->op == ONIBI_RS_ANY) {
      if (frame.pos >= RSTRING_LEN(str)) hit = 0;
        ...
        const OnibiLiteralDesc *literals = (const OnibiLiteralDesc *)(RSTRING_PTR(blob) + header.descriptors_offset);
        const OnibiLiteralDesc *literal = &literals[state->payload];
        hit = bytes[frame.pos] == (unsigned char)RSTRING_PTR(blob)[literal->data_offset];
      } else if (state->op == ONIBI_RS_CLASS) {
        const OnibiClassDesc *classes = (const OnibiClassDesc *)(RSTRING_PTR(blob) + header.classes_offset);
        const OnibiClassDesc *klass = &classes[state->payload];
        const unsigned char *bitmap = (const unsigned char *)RSTRING_PTR(blob) + klass->data_offset;
