...
onibi_rseq_lower(VALUE self, VALUE compiled)
{
    ...
    VALUE subprograms = onibi_hash_value_id(graph, id_key_subprograms);
    VALUE capture_count_value =
	onibi_hash_value_id(graph, id_key_capture_count);
    ...
    for (long i = 0; i < RARRAY_LEN(start_edges); i++) {
	long to = NUM2LONG(
	    onibi_hash_value_id(rb_ary_entry(start_edges, i), id_key_to));
	if (to < 0 || to >= state_count)
	    rb_raise(rb_eArgError,
	...
	if (!RTEST(rb_obj_frozen_p(edge)) ||
	    !RTEST(rb_obj_frozen_p(edge_actions)))
	    rb_raise(rb_eArgError,
	...
	for (long i = 0; i < RARRAY_LEN(start_edges) && physical.exec_kind == 0;
	     i++) {
	    VALUE edge_actions = onibi_hash_value_id(
		rb_ary_entry(start_edges, i), id_key_actions);
	    for (long j = 0; j < RARRAY_LEN(edge_actions); j++) {
		OnibiGActionOp code =
		    ...
		    const OnibiCounterState *counter_state, VALUE captures)
{
	...
	if (code == ONIBI_GA_TEST_COUNTER_LT ||
		...
		VALUE slot_value = onibi_hash_value_id(action, id_key_slot);
		long slot = NIL_P(slot_value) ? -1 : NUM2LONG(slot_value);
		count = slot >= 0 && (uint32_t)slot < counter_state->count
		...
		VALUE value = rb_hash_aref(
		    counters, onibi_hash_value_id(action, id_key_slot));
	...
	if (assertion == ONIBI_RAP_LOOKAHEAD) {
		...
		for (long i = 0; i < RARRAY_LEN(predicates); i++) {
		    ...
		    unsigned char byte =
			(unsigned char)RSTRING_PTR(subject)[at];
		    OnibiPredicateKind kind = (OnibiPredicateKind)NUM2UINT(
			onibi_hash_value_id(predicate, id_key_predicate_code));
		    if (kind == ONIBI_PRED_BYTE) {
			unsigned char expected = (unsigned char)NUM2INT(
			    onibi_hash_value_id(predicate, id_key_byte));
			matched = matched &&
				  (RTEST(onibi_hash_value_id(predicate,
							     id_key_ignorecase))
				       ? tolower(byte) == tolower(expected)
				       : byte == expected);
		    }
			...
			matched =
			    ...
			    (byte != '\n' || RTEST(onibi_hash_value_id(
						 predicate, id_key_multiline)));
		    }
		    else {
			VALUE bits =
			    onibi_hash_value_id(predicate, id_key_bitmap);
			matched =
			    matched && RB_TYPE_P(bits, T_STRING) &&
			    RSTRING_LEN(bits) == 32 &&
			    (((unsigned char *)RSTRING_PTR(bits))[byte >> 3] &
			     (1U << (byte & 7))) != 0;
		    }
		    if (!matched) break;
		}
		if (matched !=
		    RTEST(onibi_hash_value_id(action, id_key_positive)))
		    return 0;
		continue;
	    }
	    ...
	    if (!NIL_P(bitmap)) {
		int hit =
		    pos < length && RSTRING_LEN(bitmap) == 32 &&
		    (((unsigned char *)RSTRING_PTR(
			 bitmap))[(unsigned char)RSTRING_PTR(subject)[pos] >>
				  3] &
		     (1U << ((unsigned char)RSTRING_PTR(subject)[pos] & 7))) !=
			0;
		if (hit != RTEST(onibi_hash_value_id(action, id_key_positive)))
		    return 0;
		continue;
	    }
	    ...
	    int hit = pos + width <= length &&
		      memcmp(RSTRING_PTR(subject) + pos, RSTRING_PTR(bytes),
			     (size_t)width) == 0;
	    if (hit != RTEST(onibi_hash_value_id(action, id_key_positive)))
		return 0;
	}
	if (assertion == ONIBI_RAP_LOOKBEHIND) {
		...
		for (long i = 0; matched && i < width; i++) {
		    VALUE predicate = rb_ary_entry(predicates, i);
		    unsigned char byte =
			(unsigned char)RSTRING_PTR(subject)[pos - width + i];
		    OnibiPredicateKind kind = (OnibiPredicateKind)NUM2UINT(
			onibi_hash_value_id(predicate, id_key_predicate_code));
		    if (kind == ONIBI_PRED_BYTE) {
			unsigned char expected = (unsigned char)NUM2INT(
			    onibi_hash_value_id(predicate, id_key_byte));
			matched = RTEST(onibi_hash_value_id(predicate,
							    id_key_ignorecase))
				      ? tolower(byte) == tolower(expected)
				      : byte == expected;
		    }
		...
		int hit =
		    ...
		    (((unsigned char *)RSTRING_PTR(bitmap))
			 [(unsigned char)RSTRING_PTR(subject)[pos - 1] >> 3] &
		     (1U << ((unsigned char)RSTRING_PTR(subject)[pos - 1] &
			     7))) != 0;
		if (hit != RTEST(onibi_hash_value_id(action, id_key_positive)))
		    return 0;
		continue;
	    ...
	    long width = RSTRING_LEN(bytes);
	    int hit =
		pos >= width && memcmp(RSTRING_PTR(subject) + pos - width,
				       RSTRING_PTR(bytes), (size_t)width) == 0;
	    if (hit != RTEST(onibi_hash_value_id(action, id_key_positive)))
		return 0;
	}
    }
...
static int
onibi_ctype_casefold_hit(VALUE str, long pos, OnigCodePoint code, int ctype,
			 int hit)
{
    if (hit) return 1;
...
static int
onibi_vm_class_match(VALUE payload, VALUE str, long pos, unsigned char byte,
		     long *width)
{
    VALUE class_mode = onibi_hash_value_id(payload, id_key_class_mode);
    int encoding_index = rb_enc_get_index(str);
    if (!NIL_P(class_mode) &&
	NUM2INT(class_mode) == ONIBI_CLASS_MODE_INTERSECTION &&
	encoding_index == rb_utf8_encindex()) {
	VALUE operands = onibi_hash_value_id(payload, id_key_operands);
	if (!RB_TYPE_P(operands, T_ARRAY) || RARRAY_LEN(operands) == 0)
	    return 0;
	long common_width = 0;
	int hit = 1;
	for (long i = 0; i < RARRAY_LEN(operands); i++) {
	    ...
	    int operand_hit =
		onibi_vm_class_match(operand, str, pos, byte, &operand_width);
	    if (i == 0)
	...
	*width = common_width;
	...
	if (pos > 0 && ((unsigned char)RSTRING_PTR(str)[pos] & 0xc0) == 0x80 &&
	    (((unsigned char)RSTRING_PTR(str)[pos - 1] & 0xc0) == 0x80 ||
	     (unsigned char)RSTRING_PTR(str)[pos - 1] >= 0xc0))
	    return 0;
	OnigCodePoint code;
	long length = 0;
	...
	*width = length;
    ...
    if (NIL_P(name) && !rb_enc_str_asciionly_p(str) &&
	...
	if (RB_TYPE_P(children, T_ARRAY) && RB_TYPE_P(ranges, T_ARRAY)) {
		...
		if (kind == ONIBI_TOKEN_LITERAL) {
			...
			VALUE child_byte =
			    onibi_hash_value_id(child, id_key_byte);
			if (!NIL_P(child_byte) &&
			    code == (OnigCodePoint)NUM2INT(child_byte))
			    hit = 1;
		    }
		    else {
			const char *child_ptr = RSTRING_PTR(bytes);
			const char *child_end = child_ptr + RSTRING_LEN(bytes);
			int child_len = rb_enc_mbclen(child_ptr, child_end,
						      rb_enc_get(str));
			if (child_len > 0 &&
			    child_ptr + child_len <= child_end &&
			    ONIGENC_MBC_TO_CODE(
				rb_enc_get(str), (const OnigUChar *)child_ptr,
				(const OnigUChar *)child_end) == code)
			    hit = 1;
		    }
		}
		else if (kind == ONIBI_TOKEN_ESCAPE ||
			 kind == ONIBI_TOKEN_META_ESCAPE) {
		    VALUE child_ctype_value =
			onibi_hash_value_id(child, id_key_ctype);
		    VALUE child_byte_value =
			onibi_hash_value_id(child, id_key_byte);
		    int child_ctype = NIL_P(child_ctype_value)
					  ? -1
					  : NUM2INT(child_ctype_value);
		    if (child_ctype >= 0) {
			int child_hit = ONIGENC_IS_CODE_CTYPE(
			    rb_enc_get(str), code, child_ctype);
			if (!NIL_P(child_byte_value) &&
			    NUM2INT(child_byte_value) == 'P')
			    child_hit = !child_hit;
			if (child_hit) hit = 1;
		...
		if (RB_INTEGER_TYPE_P(rb_ary_entry(range, 0)) &&
		    RB_INTEGER_TYPE_P(rb_ary_entry(range, 1))) {
		    if (code >=
			    (OnigCodePoint)NUM2ULONG(rb_ary_entry(range, 0)) &&
			code <=
			    (OnigCodePoint)NUM2ULONG(rb_ary_entry(range, 1)))
			hit = 1;
		    continue;
		}
		if (!RB_TYPE_P(rb_ary_entry(range, 0), T_STRING) ||
		    !RB_TYPE_P(rb_ary_entry(range, 1), T_STRING))
		    continue;
		uint32_t first = 0, last = 0;
		VALUE first_bytes = rb_ary_entry(range, 0),
		      last_bytes = rb_ary_entry(range, 1);
		const char *first_ptr = RSTRING_PTR(first_bytes),
			   *last_ptr = RSTRING_PTR(last_bytes);
		const char *first_end = first_ptr + RSTRING_LEN(first_bytes),
			   *last_end = last_ptr + RSTRING_LEN(last_bytes);
		int first_len =
		    rb_enc_mbclen(first_ptr, first_end, rb_enc_get(str));
		int last_len =
		    rb_enc_mbclen(last_ptr, last_end, rb_enc_get(str));
		if (first_len > 0 && last_len > 0 &&
		    first_ptr + first_len <= first_end &&
		    last_ptr + last_len <= last_end) {
		    first = ONIGENC_MBC_TO_CODE(rb_enc_get(str),
						(const OnigUChar *)first_ptr,
						(const OnigUChar *)first_end);
		    last = ONIGENC_MBC_TO_CODE(rb_enc_get(str),
					       (const OnigUChar *)last_ptr,
					       (const OnigUChar *)last_end);
    ...
    if (NIL_P(bitmap) || !RB_TYPE_P(bitmap, T_STRING) ||
	RSTRING_LEN(bitmap) != 32)
	rb_raise(eRegexpError, "class payload has no compiled bitmap");
    *width = 1;
    return (((unsigned char *)RSTRING_PTR(bitmap))[byte >> 3] &
	    (1U << (byte & 7))) != 0;
}
