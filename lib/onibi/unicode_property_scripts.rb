...
UNICODE_LATIN_RANGES = [[65, 90], [97, 122], [170, 170], [186, 186], [192, 214], [216, 246], [248, 696], [736, 740], [7424, 7461], [7468, 7516], [7522, 7525], [7531, 7543], [7545, 7614], [7680, 7935], [8305, 8305], [8319, 8319], [8336, 8348], [8490, 8491], [8498, 8498], [8526, 8526], [8544, 8584], [11360, 11391], [42786, 42887], [42891, 42972], [42993, 43007], [43824, 43866], [43868, 43876], [43878, 43881], [64256, 64262], [65313, 65338], [65345, 65370], [67456, 67461], [67463, 67504], [67506, 67514], [122624, 122654], [122661, 122666]].freeze
    ...
    def case_ignorable?(character)
      script_match?(character, UNICODE_CASE_IGNORABLE_RANGES)
    end

    def default_ignorable?(character)
      script_match?(character, UNICODE_DEFAULT_IGNORABLE_RANGES)
    end

    def deprecated?(character)
      script_match?(character, UNICODE_DEPRECATED_RANGES)
    end

    def diacritic?(character)
      script_match?(character, UNICODE_DIACRITIC_RANGES)
    end

    def emoji_component?(character)
      script_match?(character, UNICODE_EMOJI_COMPONENT_RANGES)
    end

    def join_control?(character)
      script_match?(character, UNICODE_JOIN_CONTROL_RANGES)
    end

    def regional_indicator?(character)
      script_match?(character, UNICODE_REGIONAL_INDICATOR_RANGES)
    end

    def variation_selector?(character)
      script_match?(character, UNICODE_VARIATION_SELECTOR_RANGES)
    end

    def noncharacter_code_point?(character)
      script_match?(character, UNICODE_NONCHARACTER_CODE_POINT_RANGES)
    end

    def pattern_white_space?(character)
      script_match?(character, UNICODE_PATTERN_WHITE_SPACE_RANGES)
    end

    def quotation_mark?(character)
      script_match?(character, UNICODE_QUOTATION_MARK_RANGES)
    end

    def terminal_punctuation?(character)
      script_match?(character, UNICODE_TERMINAL_PUNCTUATION_RANGES)
    end

    def soft_dotted?(character)
      script_match?(character, UNICODE_SOFT_DOTTED_RANGES)
    end

    def extender?(character)
      script_match?(character, UNICODE_EXTENDER_RANGES)
    end

    def math?(character)
      script_match?(character, UNICODE_MATH_RANGES)
    end
