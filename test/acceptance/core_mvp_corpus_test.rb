...
class CoreMvpCorpusTest < Minitest::Test
  ...
  def test_corpus_cases_have_a_complete_shape
    ...
    assert corpus.fetch("unsupported_features").all? { |feature| !SUPPORTED_FEATURES.include?(feature) }

    corpus.fetch("cases").each do |fixture|
      assert_equal %w[encoding feature input name options outcome pattern].sort, fixture.keys.sort
