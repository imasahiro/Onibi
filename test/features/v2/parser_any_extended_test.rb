...
class V2ParserAnyExtendedTest < Minitest::Test
  def test_any_node_has_exact_value
    assert_equal Onibi::AST::Sequence.new([Onibi::AST::Any.new(".")]), Onibi::Parser.parse(".").ast
  end
  ...
  def test_extended_scoped_group_removes_comments_and_space
    ...
    assert_equal expected, Onibi::Parser.parse("(?x: a # comment\n b )").ast
  end
  ...
  def test_extended_mode_keeps_escaped_space_as_literal
    ...
    assert_equal expected, Onibi::Parser.parse("a\\ b", options: ["extended"]).ast
  end
end
