...
class V2ParserTest < Minitest::Test
  def test_parse_returns_ast_with_source_options
    result = Onibi::Parser.parse("ab|c", options: [])
  ...
  def test_parse_normalizes_public_option_forms
    assert_equal ["ignorecase"], Onibi::Parser.parse("a", options: true).options
    assert_equal %w[ignorecase multiline], Onibi::Parser.parse("a", options: "im").options
    assert_equal ["ignorecase"], Onibi::Parser.parse("a", options: Onibi::Regexp::IGNORECASE).options
  end

  def test_parse_accepts_global_inline_option_groups
    result = Onibi::Parser.parse("(?imx)cat")
  ...
  def test_parse_accepts_global_disabled_inline_options
    result = Onibi::Parser.parse("(?-imx)cat", options: %w[ignorecase multiline extended])
  ...
  def test_literal_concatenation_and_alternation_have_exact_ast_shape
    ...
    assert_equal expected, Onibi::Parser.parse("cat|dog").ast
  end
end
