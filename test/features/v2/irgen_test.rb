...
class V2IRGenTest < Minitest::Test
  ...
  def test_flat_compiler_lowers_scoped_non_digit_escape
    regexp = Onibi::Regexp.new("(?i:\\D)")
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_compiler_lowers_variable_alternation_absence
    source = "(?~(a|ab){2,})"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }
    %w[a aa aaa aab abab ababa].each do |input|
      expected = ::Regexp.new(source).match(input)
      assert_equal expected&.to_a, regexp.match(input)&.to_a, [source, input]
    end
  end

  def test_flat_compiler_lowers_non_capture_variable_alternation_absence
    source = "(?~(?:a|ab){2,})"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_compiler_lowers_outer_capture_variable_alternation_absence
    source = "(?~((?:a|ab){2,}))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }
    { "aa" => %w[a aa], "aab" => %w[a aa], "aaa" => ["aa", nil] }.each do |input, expected|
  ...
  def test_flat_compiler_lowers_variable_nested_alternation_capture_absence
    source = "(?~((a|b)+))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }
    { "aba" => ["a", "b", nil], "ababab" => ["aba", "ab", nil] }.each do |input, expected|
      assert_equal expected, regexp.match(input)&.to_a, [source, input]
    end
  end

  def test_flat_compiler_lowers_variable_nested_capture_suffix
    source = "(?~((a|b)+)c)"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_compiler_lowers_variable_non_capture_suffix
    source = "(?~(?:(?:ab|a)+)c)"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }
    { "abc" => ["ab"], "aac" => ["aa"], "c" => ["c"] }.each do |input, expected|
      assert_equal expected, regexp.match(input)&.to_a, [source, input]
    end
  end

  def test_flat_compiler_lowers_nullable_non_capture_repeat_absence
    source = "(?~(?:a*))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }
    { "" => [""], "a" => [""], "aa" => ["a"], "b" => [""] }.each do |input, expected|
      assert_equal expected, regexp.match(input)&.to_a, [source, input]
    end
  end

  def test_flat_compiler_lowers_optional_non_capture_absence
    source = "(?~a?)"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }
    { "" => [""], "a" => [""], "aa" => [""], "ba" => [""] }.each do |input, expected|
      assert_equal ::Regexp.new(source).match(input)&.to_a, regexp.match(input)&.to_a, [source, input]
      assert_equal expected, regexp.match(input)&.to_a, [source, input]
    end
  end
  ...
  def test_flat_semantic_vm_executes_scoped_unicode_bounded_capture_conditional
    source = "(?i:(?<x>é){2}(?(<x>)a|b))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    assert program.instructions.any? { |item| item.opcode == :semantic_flat }, source
    ["ééa", "Ééa", "ééb", "éé", "xééa"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_scoped_unicode_capture_conditional_suffix
    source = "(?i:(?<x>é+)(?(<x>)a|b))c"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    assert program.instructions.any? { |item| item.opcode == :semantic_flat }, source
    ["éac", "Éac", "éÉac", "ébc", "éab", "xéac"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_scoped_unicode_conditional_branches
    source = "(?i:(?<x>a)(?(<x>)é|ö))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
    ...
    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    assert program.instructions.any? { |item| item.opcode == :semantic_flat }, source
    ["aé", "aÉ", "aö", "aÖ", "a", "xaé"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_scoped_unicode_conditional_assertions
    source = "(?i:(?<x>a)(?(<x>)(?=é)|(?=ö)))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
    ...
    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    assert program.instructions.any? { |item| item.opcode == :semantic_flat }, source
    ["aé", "aö", "aÉ", "aÖ", "a", "xaé"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_scoped_unicode_conditional_lookbehind
    source = "(?i:(?<x>a)(?(<x>)(?<=é)|(?<=ö)))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
    ...
    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    assert program.instructions.any? { |item| item.opcode == :semantic_flat }, source
    ["aé", "aö", "éa", "öa", "aÉ", "aÖ", "xaé"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_empty_absence
    source = "(?~)"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_finite_nested_nullable_optional
    source = "(a??)?"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_finite_nested_nullable_non_capture_repeat
    source = "(?:a??){2,3}"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_unbounded_nested_nullable_repeat
    source = "(?:a??)*"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_greedy_nested_nullable_repeat
    source = "(?:a?)*"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_capture_unbounded_nested_nullable_repeat
    source = "(a??)*"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_capture_greedy_nested_nullable_repeat
    source = "(a?)+"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_capture_unbounded_inner_nullable_repeat
    source = "(a*)+"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_nested_possessive_repeat
    source = "a{1,3}+a"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_variable_capture_absence_probe
    source = "(?~(?:.*(ab|a)))"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
    ...
    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    assert program.instructions.any? { |item| item.opcode == :semantic_flat }, source
    ["ab", "a", "xab", "xac", "aba"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_possessive_anchor_repeat
    source = "a$++"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)
  ...
  def test_flat_semantic_vm_executes_nested_possessive_any_repeat
    source = ".{0,2}+"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    ["aaab", "x", ""].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_nested_possessive_escape_repeat
    source = "\\w{0,2}+"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    ["abc", "ab-", "-"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_anchor_literal_capture_repeat
    source = "($|a)*"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    ["a", "aa", "b", "aba"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_nested_possessive_property_repeat
    source = "\\p{Alpha}{0,2}+"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    ["Ab1", "1", "abc"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_repeated_literal_sequence_absence
    source = "(?~(?:ab){2,})"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    ["abab", "xabab", "ab", "x"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
  ...
  def test_flat_semantic_vm_executes_capture_wrapped_repeated_literal_absence
    source = "(?~(ab){2,})"
    regexp = Onibi::Regexp.new(source)
    program = regexp.send(:bytecode_program)

    refute program.instructions.any? { |item| item.opcode == :semantic_match }, source
    ["abab", "xabab", "ab"].each do |input|
      expected = ::Regexp.new(source).match(input)
      actual = regexp.match(input)
      assert_equal expected&.to_a, actual&.to_a, [source, input]
    end
