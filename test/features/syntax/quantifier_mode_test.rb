...
class QuantifierModeTest < Minitest::Test
  ...
  def test_casefold_backreference_uses_following_boundary
    source = "(s)\\1"
    expected = ::Regexp.new(source, ::Regexp::IGNORECASE).match("ſss")
    actual = Onibi::Regexp.new(source, ::Regexp::IGNORECASE).match("ſss")

    assert_equal expected.to_a, actual.to_a
    assert_equal [expected.begin(0), expected.end(0)], [actual.begin(0), actual.end(0)]
  end

  def test_casefold_backreference_does_not_cross_absolute_end
    source = "(s)\\1\\z"
  ...
  def test_nested_possessive_zero_candidates_preserve_unset_conditionals
    [
      ["(a?){0,2}+(?(1)a|b)", %w[b ba]],
      ["(a*){0,2}+(?(1)a|b)", %w[b ba]],
      ["(\\b){0,2}+(?(1)a|b)", %w[b ba]]
